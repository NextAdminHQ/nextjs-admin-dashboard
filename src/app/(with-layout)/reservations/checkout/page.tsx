...
export default function CheckOutPage() {
  const reservations = RESERVATIONS_MOCKUP.filter((reservation) => reservation.statut === "checkout");

  return (
    <>
      <Breadcrumb pageName="Check-out" />
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-dark dark:text-white">Réservations en check-out</h1>
        <p className="mt-2 text-sm text-dark-4 dark:text-dark-6">
          Liste des réservations qui ont passé la phase de check-out.
        </p>
      </div>

      <section className="overflow-hidden rounded-[10px] bg-white shadow-1 dark:bg-gray-dark dark:shadow-card">
        <div className="border-b border-stroke px-6 py-4 dark:border-dark-3">
          <h2 className="text-base font-semibold text-dark dark:text-white">Sorties récentes</h2>
          <p className="mt-1 text-sm text-dark-4 dark:text-dark-6">{reservations.length} réservation(s) en check-out.</p>
        </div>
