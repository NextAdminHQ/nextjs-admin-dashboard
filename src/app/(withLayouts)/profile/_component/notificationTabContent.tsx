import { Toggle } from "@/components/tailgrids/core/toggle";

const notificationSettings = [
  { id: "email-notification", label: "Email notification", defaultChecked: true },
  { id: "push-notification", label: "Push Notification", defaultChecked: false },
  { id: "product-update", label: "Product Update", defaultChecked: true },
  { id: "marketing-email", label: "Marketing Email", defaultChecked: false },
  { id: "security-alert", label: "Security Alert", defaultChecked: true },
] as const;

export default function NotificationTabContent() {
  return (
    <div>
      <h2 className="text-xl leading-7 font-semibold text-text-primary mb-6">Notification</h2>

      <div className="divide-y divide-border-secondary-alt">
        {notificationSettings.map(({ id, label, defaultChecked }) => (
          <div key={id} className="flex items-center justify-between gap-4 py-5">
            <span
              id={`${id}-label`}
              className="text-sm font-medium leading-5 block text-text-primary"
            >
              {label}
            </span>

            <Toggle
              aria-labelledby={`${id}-label`}
              defaultChecked={defaultChecked}
              size="md"
              className="shrink-0"
            />
          </div>
        ))}
      </div>
    </div>
  );
}
