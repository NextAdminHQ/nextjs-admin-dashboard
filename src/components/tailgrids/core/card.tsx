...
export function CardTitle({ children, className }: HeadingProps) {
  return (
    <Heading
      className={cn("leading-6 font-semibold tracking-[-0.2px] text-text-primary", className)}
    >
      {children}
    </Heading>
  );
...
export function CardDescription({ children, className }: ComponentProps<"div">) {
  return (
    <div
      className={cn("mt-0.5 text-base leading-6 tracking-[-0.2px] text-text-secondary", className)}
    >
      {children}
    </div>
  );
